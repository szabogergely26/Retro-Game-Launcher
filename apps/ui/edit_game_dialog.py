...
from apps.core.desktop_writer import create_desktop_icon_launcher
...
class EditGameDialog(QDialog):
    def __init__(self, game: dict, parent: QWidget | None = None) -> None:
        ...
        self.setWindowTitle("Játék szerkesztése")
        ...
        self._create_desktop_icon_requested = False

        self._desktop_icon_path = self._original_game.get("desktop_icon_path", "")


        create_desktop_icon_button = QPushButton("Méntés és asztali ikon létrehozása")
        create_desktop_icon_button.clicked.connect(self._accept_with_desktop_icon)
        ...
        main_layout.addWidget(QLabel("Név:"))
        main_layout.addWidget(self.name_edit)

        main_layout.addSpacing(8)

        main_layout.addWidget(QLabel("Indítófájl:"))
        ...
        buttons_layout.addWidget(create_desktop_icon_button)
    ...
    def data(self) -> dict:
        ...
        updated_game["desktop_icon_path"] = self._desktop_icon_path
    ...
    def _choose_icon(self) -> None:
        start_dir = str(Path(self._icon_path).parent) if self._icon_path else str(Path.home())
    ...
    def _accept_if_valid(self) -> None:
        ...
        if not Path(executable_path).exists():
            answer = QMessageBox.question(
                self,
                "Nem létező indítófájl",
                "Az indítófájl nem található.\n\nÍgy is elmented?",
    ...
    def _create_desktop_icon(self) -> None:
        name = self.name_edit.text().strip()
        executable_path = self.executable_edit.text().strip()
        ...
        if not name:
            QMessageBox.warning(
                self,
                "Hiányzó név",
                "Add meg a játék nevét az asztali ikon létrehozásához.",
            )
            return
        ...
        if not executable_path:
            QMessageBox.warning(
                self,
                "Hiányzó indítófájl",
                "Add meg az indítófájlt az asztali ikon létrehozásához.",
            )
            return
        ...
        desktop_icon_path = create_desktop_icon_launcher(
            ...
            icon_path=icon_path,
            launcher_type=self._original_game.get("type", "native"),
        )

        self._desktop_icon_path = str(desktop_icon_path)
    ...
    def _accept_with_desktop_icon(self) -> None:
        self._create_desktop_icon_requested = True
        self._accept_if_valid()



    def should_create_desktop_icon(self) -> bool:
        return self._create_desktop_icon_requested
